...
class BaseSegmentor(BaseModule, metaclass=ABCMeta):
    ...
    def show_result(self,
        ...
        # jjh : Save result masks
        self.MICCAI_SaveResultMasks(seg, out_file)
        ...
        if not (show or out_file):
            ...
            return img
    # jjh : Save result masks
    def MICCAI_SaveResultMasks(self, seg, out_file):

        for i in range(3):
            cls = i + 1
            mask = (seg == cls) * 1
            savePath = out_file.replace('\\', '\\MICCAI\\'+str(cls)+'\\')
            mmcv.imwrite(mask, savePath)
